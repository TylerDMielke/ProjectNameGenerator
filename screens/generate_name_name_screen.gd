...
func _load_existing_names():
	...
	if not FileAccess.file_exists(Constants.NAME_FILE_PATH):
		return

	# Read names file
	var file = FileAccess.open(Constants.NAME_FILE_PATH, FileAccess.READ)
...
func _on_confirm_button_down():
	# save text to file
	var file = FileAccess.open(Constants.NAME_FILE_PATH, FileAccess.WRITE)
